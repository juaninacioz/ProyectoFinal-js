...
const GetRequestGenerica = async (urlBase, parametros) => {
  ...
  return fetch(url, requestOpciones)
    .then((response) =>
      response.json()
    ) /*Transformamos la respuesta a formato json para poder manipularlo adecuadamente*/
    .then(
      (result) => result
    ) /*Retonamos el resultado en caso de exito en la resolucion de la promesa*/
    .catch((error) =>
      console.log("Error:", error)
    ); /*Retornamos el error en caso de fracaso en la resolucion de la promesa*/
};
...
const MapearProductos = (productos, precioXScoreAlimenticio) => {
  const productosModificados = productos.products.map((producto) => {
    ...
    return {
      ...
      precio: Math.abs(
        producto.nutriments["nutrition-score-fr_100g"] * precioXScoreAlimenticio
      ),
...
const ObtenerProductosDeCategoria = async (categoria) => {
  let productos = await GetRequestGenerica(
    `https://es.openfoodfacts.org/categoria/${categoria}.json`
  ); /***/
...
function crearCard(prod) {
  ...
  <div class="card" style="width: 18rem;">
   <img src="${prod.imagen}" class="card-img-top img-prod " alt=${prod.nombre}>
   <div class="card-body">
    <h5 class="card-title">${prod.nombre}</h5>
    <p class="card-text">${prod.marca}</p>
    <p class="card-text">${prod.peso}</p>
    <p class="card-text">${prod.precio}</p>
    <button href="#"class="btn btn-primary" id="bt-${prod.id}">Agregar</button> 
   </div>
  </div>
...
function comprar() {
  productos.forEach((prod) =>
    document.getElementById(`bt-${prod.id}`).addEventListener(`click`, () => {
      console.log(prod);
    })
  );
}
...
function crearCarritoContent(prod) {
  ...
  <div class="card-body">
   ...
   <p class="card-text">${prod.peso}</p>
   <p class="card-text">${prod.precio}</p>
   <button href="#"class="btn btn-primary" id="bt-eliminar">Eliminar</button> 
  </div>
 </div>
  `;
